...
const HIDDEN_CONFIGS_FILE_PATH = "data/hidden-configs.json";
const HIDDEN_REPOS_FILE_PATH = "data/hidden-repos.json";

async function loadHiddenConfigs() {
  const repo = HIDDEN_FILE_REPO;
  const path = HIDDEN_CONFIGS_FILE_PATH;
  try {
    const data = await githubFetch(`/repos/${repo}/contents/${path}`);
    if (data && data.content) {
      const text = Buffer.from(data.content, "base64").toString("utf8");
      const parsed = JSON.parse(text);
      return {
        hiddenConfigIds: Array.isArray(parsed.hiddenConfigIds) ? parsed.hiddenConfigIds : [],
        hiddenRepoIds: Array.isArray(parsed.hiddenRepoIds) ? parsed.hiddenRepoIds : [],
        updatedAt: parsed.updatedAt || null
      };
    }
  } catch (err) {
    if (err.statusCode !== 404) {
      console.error("Failed to load hidden configs", err && err.message);
    }
  }
  return { hiddenConfigIds: [], hiddenRepoIds: [], updatedAt: null };
}

async function saveHiddenConfigs(hidden, attempt = 0) {
  const repo = HIDDEN_FILE_REPO;
  const path = HIDDEN_CONFIGS_FILE_PATH;
  let sha;
  try {
  ...
  const contentObj = {
    hiddenConfigIds: hidden.hiddenConfigIds,
    hiddenRepoIds: hidden.hiddenRepoIds,
    updatedAt: new Date().toISOString()
  };
  const content = Buffer.from(JSON.stringify(contentObj, null, 2)).toString("base64");
  const body = {
    message: "console: update hidden configs/repos",
    content,
    ...(sha ? { sha } : {})
  };
  try {
    ...
    if ((err.statusCode === 409 || err.statusCode === 422) && attempt < 5) {
      return saveHiddenConfigs(hidden, attempt + 1);
    }
    throw err;
  }
}

async function addHiddenConfig(configId) {
  const hidden = await loadHiddenConfigs();
  const id = safeString(configId);
  if (!id || hidden.hiddenConfigIds.includes(id)) return hidden;
  hidden.hiddenConfigIds.push(id);
  await saveHiddenConfigs(hidden);
  return hidden;
}

async function removeHiddenConfig(configId) {
  const hidden = await loadHiddenConfigs();
  const id = safeString(configId);
  hidden.hiddenConfigIds = hidden.hiddenConfigIds.filter(x => x !== id);
  await saveHiddenConfigs(hidden);
  return hidden;
}

async function addHiddenRepo(repoId) {
  const hidden = await loadHiddenConfigs();
  const rid = safeString(repoId);
  if (!rid || hidden.hiddenRepoIds.includes(rid)) return hidden;
  hidden.hiddenRepoIds.push(rid);
  await saveHiddenConfigs(hidden);
  return hidden;
}

async function removeHiddenRepo(repoId) {
  const hidden = await loadHiddenConfigs();
  const rid = safeString(repoId);
  hidden.hiddenRepoIds = hidden.hiddenRepoIds.filter(x => x !== rid);
  await saveHiddenConfigs(hidden);
  return hidden;
}
